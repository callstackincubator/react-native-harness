...
| `permissions`                      | Enable platform-specific permission prompt automation (default: `false`). On iOS, this controls whether Harness starts the agent-device permission watchdog. Requires Node.js 22.12 or newer. |
...
When `permissions` is `false`, Harness does not start platform-specific permission automation helpers. On iOS that means no agent-device UI test runner is prepared for permission auto-accept. Physical iOS devices still require `device.codeSign` in the runner config when you set `permissions: true`, and personal teams may also need `device.codeSign.runnerBundleId`. See the [permissions guide](/docs/guides/permissions).
