...
The recommended way to run Harness is through Jest, which means you get all the great features of the Jest CLI including watch mode, code coverage, filtering, and more. The standalone React Native Harness CLI is going to be deprecated in the near future, so we strongly encourage using the Jest integration.
...
Now you're ready to run your tests! Simply use Jest to execute your Harness tests:

<PackageManagerTabs command="jest" />

Since Harness runs through Jest, you get all the powerful Jest CLI features out of the box:

- **Watch mode**: `jest --watch` to automatically rerun tests when files change
- **Code coverage**: `jest --coverage` to see how much of your code is tested
- **Run specific tests**: `jest MyComponent.harness` to run only certain test files

:::note Legacy CLI
The standalone `react-native-harness test` CLI command is still available but will be deprecated soon. We recommend using Jest directly for the best experience.
:::
