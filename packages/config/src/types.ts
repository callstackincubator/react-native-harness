...
export type NativeTestRunnerConfig = z.infer<typeof NativeTestRunnerConfigSchema>;
...
export function isNativeRunnerConfig(config: TestRunnerConfig): config is NativeTestRunnerConfig {
  return config.platform === 'ios' || config.platform === 'android';
}

export function isWebRunnerConfig(config: TestRunnerConfig): config is WebTestRunnerConfig {
  return config.platform === 'web';
}

export function assertNativeRunnerConfig(config: TestRunnerConfig): asserts config is NativeTestRunnerConfig {
  if (!isNativeRunnerConfig(config)) {
    throw new Error(`Expected native runner config but got platform: ${config.platform}`);
  }
}

export function assertWebRunnerConfig(config: TestRunnerConfig): asserts config is WebTestRunnerConfig {
